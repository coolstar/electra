// https://gist.github.com/summertriangle-dev/9eb4811fb728791b10fb6438e32a2cb9
// build using xcrun -sdk iphoneos cc -dynamiclib -arch arm64 substrate-shim.c -L$PWD -lsubstitute -lobjc -install_name /Library/Frameworks/CydiaSubstrate.framework/CydiaSubstrate -o CydiaSubstrate
...
void MSHookMessageEx(Class _class, SEL sel, IMP imp, IMP *result) {
	if (class_getInstanceMethod(_class, sel) || class_getClassMethod(_class, sel)) {
		...
		os_log_error(OS_LOG_DEFAULT, "shim: Tried to hook non-existent selector %s on class %s",
			sel_getName(sel), class_getName(_class));
			if (result) *result = NULL;
	}
}

// i don't think anyone uses this function anymore, but it's here for completeness :thinking:
